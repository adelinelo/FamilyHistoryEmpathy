...
#openendedbytreatment.csv --handcoded variables
open.ended<-read.csv("openendedbytreatment.csv") #match back on rownames with d
...
open.ended$Imm_reason_text_01<-ifelse(!is.na(open.ended$Imm_reason_text),1,0)#is there text
...
table(open.ended$Family_History_Treatment,is.na(open.ended$Imm_reason_text),useNA='always')
```

[Of respondents who received Control, 55.3% (n=1039) wrote family history descriptions. Of respondents who received Treatment, 54.8% (n=1076) wrote family history descriptions -- essentially the equivalent proportion. ]{color="navy"}

```{r,echo=TRUE,eval=TRUE,message=FALSE,warning=FALSE}

textmod1<-(lm(Imm_reason_text_01~Family_History_Treatment + partyid2 + race + gender + age + educ + Family_History_Treatment*partyid2 + Family_History_Treatment*race + Family_History_Treatment*gender + Family_History_Treatment*age + Family_History_Treatment*educ,data=open.ended))
tab_model(textmod1, auto.label=TRUE)
```

Possibly only difference is that gender-other more likely to write text under Treatment compared to under Control arms -- though note this is a total of 10 respondents 4 of whom didn't write and 6 who did.
